...
Redmine::Plugin.register :redmine_import_issues do
  ...
  project_module :redmine_import_issues do
    permission :import_issues, :import_issues => [:index, :prepare,:recover, :download, :overview, :create, :change_file, :load_values_for_field, :load_values_for_time_entry, :update, :validate, :import]
    permission :import_issues_delete_templates, :import_issues => [:destroy]
    menu :project_menu, :import_issues, { :controller => 'import_issues', :action => 'index' }, :caption => :label_import_issues, :param => :project_id
  end
