...
class ImportAction < ActiveRecord::Base
  ...
  def get_core_value(field, value)
    v = value
    case field
    when "status_id"
      v = IssueStatus.find_by_name(value).id rescue 0
    else
      v = value
    end
    v
  ...
  def required_fields
    ["subject", "priority_id", "status_id"] + tracker.custom_fields.select {|cf| cf.is_required?}.map{|cf| cf.id.to_s}
  end
  ...
  def build_complete_mapping(file_row, issue_template = nil)
    ...
    tmp.each{ |t,v|
       tmp[t] = get_core_value(t, file_row[v.to_i][:value].to_s)
    }
    new_issue.merge!(tmp)
  ...
  def get_value_with_format_from_file(t, v, file_row)
    ...
    format ||= "string"
    accept_multiple = (format == "list" && @@formats[t]["multiple"])
